...
/// Reads an object property by its two-letter short code. A null object exists "0" times; all of
/// its other properties read as 0. lf/ac/dr only apply to living objects.
...
{
    if (param.size() == 5) {
        const int value = param_to_int(param.substr(1));
